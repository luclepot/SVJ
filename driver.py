...
def setup_parser():
    ...
    for subparser in [select, convert, train]:
        ...
        subparser.add_argument('-f', '--filter', dest='filter', action='store', default='*', help='glob-style filter for root files in inputfile')
    ...
    convert.add_argument('-r', '--range', dest='range', action='store', type=_range_input, default=None, help='range of data to parse')


    return parser

def convert_main(inputdir, outputdir, name, filter, range, DR, NC):
    print "convert main"
    sys.exit(0)
